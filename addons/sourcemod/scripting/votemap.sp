...
#define MAXLENGTH_MAP_FILENAME     64
...
/**
  ...
  *
  * @param szName     Library name
  */
public void OnLibraryRemoved(const char[] szName)
{
    if (StrEqual(szName, LIB_CHANGELEVEL)) {
    ...
    switch (action)
    {
        case VoteAction_Display:
        {
            ...
            if (TranslationPhraseExists(map.displayname)) {
                char szDisplayName[MAXLENGTH_MAP_DISPLAYNAME];
                FormatEx(szDisplayName, sizeof(szDisplayName), "%T", map.displayname, iClient);
                hVote.SetDetails("%T", "VOTE_TITLE", iClient, szDisplayName);
            } else {
                hVote.SetDetails("%T", "VOTE_TITLE", iClient, map.displayname);
            }
 ...
 *
 * If l4d2_changelevel native L4D2_ChangeLevel is available, it will be used.
